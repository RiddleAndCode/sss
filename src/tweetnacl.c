...
static u32 L32(u32 x, int c) {
  return (x << (unsigned int)c) | ((x & 0xffffffffu) >> (32u - (unsigned int)c));
}
...
sv pack25519(u8 *o, const gf n) {
  ...
  FOR(j, 2) {
    m[0] = t[0] - 0xffed;
    for (i = 1; i < 15; i++) {
      m[i] = t[i] - 0xffff - (((u64)m[i - 1] >> 16u) & 1u);
      m[i - 1] &= 0xffff;
    }
    m[15] = t[15] - 0x7fff - (((u64)m[14] >> 16u) & 1u);
    int b = ((u64)m[15] >> 16u) & 1u;
    m[14] &= 0xffff;
    sel25519(t, m, 1 - b);
  }
  FOR(i, 16) {
    o[2 * i] = (u64)t[i] & 0xffu;
    o[2 * i + 1] = (uint64_t)t[i] >> 8u;
  }
}
...
int crypto_scalarmult(u8 *q, const u8 *n, const u8 *p) {
  ...
  a[0] = d[0] = 1;
  for (i = 254; i >= 0; --i) {
    i64 r = (u64)(z[(uint64_t)i >> 3u] >> ((uint64_t)i & 7u)) & 1u;
    sel25519(a, b, r);
    sel25519(c, d, r);
    A(e, a, c);
    Z(a, a, c);
...
static u64 R(u64 x, int c) {
  return (x >> (unsigned int)c) | (x << (64u - c));
}
...
static u64 sigma0(u64 x) {
  return R(x, 1) ^ R(x, 8) ^ (x >> 7u);
}
static u64 sigma1(u64 x) {
  return R(x, 19) ^ R(x, 61) ^ (x >> 6u);
}
...
sv scalarmult(gf p[4], gf q[4], const u8 *s) {
  ...
  set25519(p[3], gf0);
  for (i = 255; i >= 0; --i) {
    u8 b = (unsigned int)(s[i / 8] >> ((unsigned int)i & 7u)) & 1u;
    cswap(p, q, b);
    add(q, p);
    add(p, p);
    cswap(p, q, b);
...
sv modL(u8 *r, i64 x[64]) {
  ...
  for (i = 63; i >= 32; --i) {
    carry = 0;
    for (j = i - 32; j < i - 12; ++j) {
      x[j] += (uint64_t)carry - 16 * x[i] * L[j - (i - 32)];
      carry = (uint64_t)(x[j] + 128) >> 8u;
      x[j] -= (uint64_t)carry << 8u;
    }
    x[j] += carry;
    x[i] = 0;
  }
  carry = 0;
  FOR(j, 32) {
    x[j] += carry - ((uint64_t)x[31] >> 4u) * L[j];
    carry = (uint64_t)x[j] >> 8u;
    x[j] &= 255;
  }
  FOR(j, 32)
  x[j] -= carry * L[j];
  FOR(i, 32) {
    x[i + 1] += (uint64_t)x[i] >> 8u;
    r[i] = (uint64_t)x[i] & 255u;
  }
}
